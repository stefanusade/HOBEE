...
include "verify.php";
    if(isset($_POST['submit'])){
        include "../../../config/db.php";
			$old = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM supplier WHERE username='$username'"));
			...
			if(in_array($eks, $ekstensi) === true){
				if($ukuran < 1044070){			
					...
					$query = mysqli_query($conn,"UPDATE supplier SET foto_profil='$nama' WHERE username='$username'");
					if($query){
					    unlink($foto);
